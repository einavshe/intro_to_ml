...
def make_dataset(dir, class_to_idx):
    ...
    for target in sorted(os.listdir(dir)):
        ...
        """# save mini data
        # import shutil
        # cls_dir = d.replace("data", "mini_data")
        # os.makedirs(cls_dir, exist_ok=True)
        # fnames = os.listdir(d)
        # for fn in sorted(fnames)[:10]:
        #     if is_audio_file(fn):
        #         path = os.path.join(d.replace("\\","/"), fn)
        #         new_p = os.path.join(cls_dir, fn)
...
class GCommandLoader(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, window_size=.02,
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            path, target = self.spects[index], 0
